...
import {
  validateTagName, normalizeTagName, dedupeSort, parseTagCell, formatTagCell,
  splitKnownUnknown, itemMatchesTags, suggestTags, usageCounts,
  loadTagRegistry, createTag, deleteTag, mergeTags, categoriesToTags, migrateCategories,
} from "./tags.js";
...
let searchType = "text";
let query = "";
// Tags replace the single free-text category. Selected filter tags combine with
// AND — each chip narrows the list rather than widening it.
let TAG_REGISTRY = [];       // [{name}] sorted
let selectedFilterTags = []; // browse filter
let formTags = [];           // tags on the item currently being edited
let editingId = null;
let formPhoto = null;
...
// Placeholder tint and chart colours key off the first tag now that category is
// gone. Sorted tags mean the colour is stable rather than depending on the order
// they happened to be clicked.
function colorForItem(it) {
  const first = (it && it.tags && it.tags.length) ? it.tags[0] : null;
  return first ? colorFor(first) : "#9A8F82";
}
...
function ensureAvgRgb(it) {
  ...
  if (!ok) it.avgRgb = mixColor(colorForItem(it), "#FFFFFF", 0.55);
  return it.avgRgb;
}
function toast(msg) {
...
const SEED_ITEMS = [
  { id: "FRN-001", sku: "FRN-001", name: "Oakwood 3-Seater Sofa", tags: ["sofas"], material: "Oak & Linen", supplier: "WoodCraft Suppliers", warehouse: "Warehouse A", cost: 18000, price: 27500, stock: 12, reorder: 5, image: null },
  { id: "FRN-002", sku: "FRN-002", name: "Maple Dining Table (6-seat)", tags: ["dining"], material: "Maple Wood", supplier: "Timberline Furnishings", warehouse: "Warehouse A", cost: 15500, price: 23900, stock: 6, reorder: 4, image: null },
  { id: "FRN-003", sku: "FRN-003", name: "Elmwood Queen Bed Frame", tags: ["bedroom"], material: "Elm Wood", supplier: "WoodCraft Suppliers", warehouse: "Warehouse B", cost: 12000, price: 19500, stock: 3, reorder: 5, image: null },
  { id: "FRN-004", sku: "FRN-004", name: "Cushion Armchair - Grey", tags: ["chairs"], material: "Fabric & Pine", supplier: "ComfortSeating Co.", warehouse: "Warehouse A", cost: 4200, price: 7500, stock: 20, reorder: 8, image: null },
  { id: "FRN-005", sku: "FRN-005", name: "Glass Top Coffee Table", tags: ["tables"], material: "Glass & Steel", supplier: "Timberline Furnishings", warehouse: "Warehouse A", cost: 3200, price: 5800, stock: 15, reorder: 6, image: null },
  { id: "FRN-006", sku: "FRN-006", name: "Teakwood Bookshelf 5-Tier", tags: ["storage"], material: "Teak Wood", supplier: "WoodCraft Suppliers", warehouse: "Warehouse B", cost: 6800, price: 10900, stock: 2, reorder: 4, image: null },
  { id: "FRN-007", sku: "FRN-007", name: "Recliner Sofa - Leather", tags: ["sofas"], material: "Leather", supplier: "ComfortSeating Co.", warehouse: "Warehouse A", cost: 21000, price: 32000, stock: 4, reorder: 3, image: null },
  { id: "FRN-008", sku: "FRN-008", name: "Wardrobe 3-Door", tags: ["bedroom"], material: "MDF & Laminate", supplier: "Timberline Furnishings", warehouse: "Warehouse B", cost: 13500, price: 21000, stock: 7, reorder: 5, image: null },
  { id: "FRN-009", sku: "FRN-009", name: "Study Desk with Drawer", tags: ["office"], material: "Engineered Wood", supplier: "ComfortSeating Co.", warehouse: "Warehouse A", cost: 5200, price: 8900, stock: 0, reorder: 5, image: null },
  { id: "FRN-010", sku: "FRN-010", name: "Outdoor Patio Chair Set (2)", tags: ["outdoor"], material: "Rattan & Aluminium", supplier: "GreenScape Furniture", warehouse: "Warehouse C", cost: 7600, price: 12500, stock: 10, reorder: 6, image: null },
].map((it) => ({ ...it, avgRgb: mixColor(colorForItem(it), "#FFFFFF", 0.55) }));
...
function render() {
  ...
  if (typeof refreshMigrateTagsButton === "function") refreshMigrateTagsButton();
  document.getElementById("btnItemTags").classList.toggle("hidden", mode !== "manager");
...
function renderCategoryChips() {
  // Chips come from the defined tag registry, not from whatever happens to be
  // typed on items — that inversion is the whole point of the tag system.
  const chips = TAG_REGISTRY.map((t) => {
    const on = selectedFilterTags.includes(t.name);
    return `<button class="chip ${on ? "active" : ""}" data-tag="${esc(t.name)}" style="text-transform:capitalize">${esc(t.name)}</button>`;
  });
  const clear = selectedFilterTags.length
    ? `<button class="chip" data-tag="__clear">Clear (${selectedFilterTags.length})</button>` : "";
  document.getElementById("catChips").innerHTML = clear + chips.join("");
}

function getFiltered() {
  const q = query.trim().toLowerCase();
  return ITEMS.filter((it) => {
    // Category is searchable now too. Typing "dining" found nothing before,
    // because the text search only looked at name, SKU, supplier and material.
    const matchQ = !q || [it.name, it.sku, it.supplier, it.material, it.warehouse]
      .some((v) => (v || "").toLowerCase().includes(q))
      || (it.tags || []).some((t) => t.includes(q));
    return matchQ && itemMatchesTags(it.tags, selectedFilterTags);
  });
}
...
function analyzeUploadedPhoto(dataUrl) {
  ...
  img.onload = () => {
    ...
    document.getElementById("matchList").innerHTML = ranked.slice(0, 6).map((it, idx) => {
      const s = stockInfo(it);
      const barColor = idx === 0 ? "#A8562E" : colorForItem(it);
      return `
    ...
    wrap.innerHTML = `
      <div class="qr-found-card">
        ...
        <div style="flex:1;min-width:0;" class="sans">
          ...
          <div style="font-size:11.5px;color:var(--muted);margin-top:3px;">${esc(found.sku)}${(found.tags || []).length ? " · " + esc(formatTagCell(found.tags)) : ""}</div>
...
function openAddModal() {
  formTags = [];
  const ts = document.getElementById("tagSearch");
  if (ts) ts.value = "";
  renderChosenTags();
  const sg = document.getElementById("tagSuggest");
  ...
  ["sku", "name", "material", "supplier", "warehouse", "cost", "price", "stock", "reorder"].forEach((k) => {
...
// ============================================================================
// Tag picker — search, click to select, chips with a cross to remove.
// Deliberately cannot create tags: the input filters the registry and nothing
// else. That constraint is what stops the catalogue drifting the way free-text
// categories did.
// ============================================================================
let tagSuggestIndex = -1;

function renderChosenTags() {
  document.getElementById("tagChosen").innerHTML = formTags.map((t) =>
    `<span class="tag-pill">${esc(t)}<button type="button" data-remove-tag="${esc(t)}" aria-label="Remove ${esc(t)}">×</button></span>`
  ).join("");
}
...
function renderTagSuggestions() {
  ...
  const matches = suggestTags(TAG_REGISTRY, q, formTags);

  if (!TAG_REGISTRY.length) {
    box.innerHTML = `<div class="tag-empty">No tags defined yet — add some in Manage → Item Tags.</div>`;
  } else if (!matches.length) {
    // Says plainly that nothing will be created, so the dead end is understood
    // rather than looking like a bug.
    box.innerHTML = `<div class="tag-empty">No matching tag. Tags can only be created in Manage → Item Tags.</div>`;
  } else {
    box.innerHTML = matches.map((t, i) =>
      `<div class="tag-option ${i === tagSuggestIndex ? "active" : ""}" data-add-tag="${esc(t.name)}">${esc(t.name)}</div>`
    ).join("");
  }
...
function addFormTag(name) {
  ...
  if (!TAG_REGISTRY.some((x) => x.name === t)) return; // never accept an undefined tag
  formTags = dedupeSort([...formTags, t]);
...
function removeFormTag(name) {
  formTags = formTags.filter((t) => t !== normalizeTagName(name));
  renderChosenTags();
  renderTagSuggestions();
}

document.getElementById("tagChosen").onclick = (e) => {
  const btn = e.target.closest("[data-remove-tag]");
  if (btn) removeFormTag(btn.dataset.removeTag);
};
document.getElementById("tagSuggest").onclick = (e) => {
  const opt = e.target.closest("[data-add-tag]");
  if (opt) addFormTag(opt.dataset.addTag);
};
document.getElementById("tagSearch").oninput = () => { tagSuggestIndex = -1; renderTagSuggestions(); };
document.getElementById("tagSearch").onfocus = renderTagSuggestions;
document.getElementById("tagSearch").onkeydown = (e) => {
  const opts = [...document.querySelectorAll("#tagSuggest [data-add-tag]")];
  if (e.key === "ArrowDown" || e.key === "ArrowUp") {
    e.preventDefault();
    if (!opts.length) return;
    tagSuggestIndex = e.key === "ArrowDown"
      ? (tagSuggestIndex + 1) % opts.length
      : (tagSuggestIndex - 1 + opts.length) % opts.length;
    renderTagSuggestions();
  } else if (e.key === "Enter") {
    // Enter picks the highlighted suggestion. It never creates a tag, which is
    // why a bare Enter with nothing highlighted does nothing at all.
    e.preventDefault();
    if (tagSuggestIndex >= 0 && opts[tagSuggestIndex]) addFormTag(opts[tagSuggestIndex].dataset.addTag);
    else if (opts.length === 1) addFormTag(opts[0].dataset.addTag);
  } else if (e.key === "Escape") {
    document.getElementById("tagSuggest").classList.add("hidden");
  }
};
...
async function submitForm() {
  ...
  const tags = dedupeSort(formTags);
  ...
  const item = {
    ...
    avgRgb: formPhoto ? formPhoto.avgRgb : mixColor(colorFor(tags[0] || ""), "#FFFFFF", 0.55),
  };
...
async function handleExcelImport(file) {
  ...
  try {
    ...
    const parsed = await mapLimit(rows, 4, async ({ r, aoaIndex }) => {
      const tags = parseTagCell(r[col("Tags")] !== undefined ? r[col("Tags")] : r[col("Category")]);
      ...
      return {
        ...
        tags, material: String(r[col("Material")] || ""), supplier: String(r[col("Supplier")] || ""),
        ...
        image, avgRgb: avgRgb || mixColor(colorFor(tags[0] || ""), "#FFFFFF", 0.55),
      };
    });
    ...
    // Unknown tags BLOCK the import outright. Silently dropping them would let a
    // typo like "dinning" quietly strip an item's tags with no signal at all.
    const registryNames = new Set(TAG_REGISTRY.map((t) => t.name));
    const unknownTags = new Map(); // tag -> [skus]
    for (const it of parsed) {
      const { unknown } = splitKnownUnknown(it.tags, registryNames);
      for (const u of unknown) {
        if (!unknownTags.has(u)) unknownTags.set(u, []);
        if (unknownTags.get(u).length < 4) unknownTags.get(u).push(it.sku);
      }
    }
    if (unknownTags.size) {
      const lines = [...unknownTags.entries()].map(([tag, skus]) =>
        `<li><b>${esc(tag)}</b> — e.g. ${esc(skus.join(", "))}</li>`).join("");
      showImportModal(
        `<b>Import stopped — ${unknownTags.size} tag(s) in this file aren't defined.</b>` +
        `<ul>${lines}</ul>` +
        `Nothing was imported and your inventory is unchanged. Either correct the spelling in the ` +
        `file, or add these in <b>Manage → Item Tags</b> first and import again. ` +
        `The <b>Tags</b> worksheet in the export lists every valid tag.`,
        true
      );
      return;
    }
...
async function handleExcelExport() {
  ...
  try {
    ...
    const headers = ["SKU", "Image", "Item Name", "Tags", "Material", "Supplier",
    ...
    for (const it of ITEMS) {
      ...
      const row = ws.addRow([
        it.sku, "", it.name, formatTagCell(it.tags), it.material, it.supplier, it.warehouse,
    ...
    // A second worksheet listing every valid tag. It travels with the file, so
    // the allowed values are available even with the app closed, and it's what
    // the conditional formatting below checks against.
    const tagSheet = wb.addWorksheet("Tags");
    tagSheet.addRow(["Tag"]);
    tagSheet.getRow(1).font = { bold: true, color: { argb: "FFFFFFFF" } };
    tagSheet.getRow(1).fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF1F2937" } };
    tagSheet.columns = [{ width: 24 }];
    TAG_REGISTRY.forEach((t) => tagSheet.addRow([t.name]));
    tagSheet.addRow([]);
    tagSheet.addRow(["This list is regenerated on every export."]);
    tagSheet.addRow(["Editing it does nothing — tags are created in Manage → Item Tags."]);

    // Flags a Tags cell red as soon as it contains a tag that isn't defined,
    // so typos are caught while editing rather than at import.
    if (TAG_REGISTRY.length) {
      const lastTagRow = TAG_REGISTRY.length + 1;
      ws.addConditionalFormatting({
        ref: `D2:D${Math.max(rowNum, 2)}`,
        rules: [{
          type: "expression",
          formulae: [`AND(LEN(D2)>0,SUMPRODUCT(--ISNUMBER(MATCH(TRIM(MID(SUBSTITUTE($D2,",",REPT(" ",99)),(ROW(INDIRECT("1:"&(LEN($D2)-LEN(SUBSTITUTE($D2,",",""))+1)))-1)*99+1,99)),Tags!$A$2:$A$${lastTagRow},0)))<>LEN($D2)-LEN(SUBSTITUTE($D2,",",""))+1)`],
...
document.getElementById("catChips").onclick = (e) => {
  ...
  if (tag === "__clear") selectedFilterTags = [];
  else if (selectedFilterTags.includes(tag)) selectedFilterTags = selectedFilterTags.filter((t) => t !== tag);
  else selectedFilterTags = [...selectedFilterTags, tag];
...
// ============================================================================
const GOOGLE_CLIENT_ID = (firebaseConfig.googleClientId || "").trim();

// A real Google OAuth client ID always ends in .apps.googleusercontent.com.
// Without this check the placeholder was sent to Google verbatim, which bounced
// the user to an "Access blocked: invalid_client" page that says nothing about
// the actual problem being an unconfigured file.
function clientIdProblem(id) {
  if (!id || id === "PASTE_YOUR_GOOGLE_CLIENT_ID") return "not-set";
  if (!id.endsWith(".apps.googleusercontent.com")) return "malformed";
  if (id.includes(" ")) return "malformed";
  return null;
}
...
async function startSync() {
  ...
  const problem = clientIdProblem(GOOGLE_CLIENT_ID);
  if (problem) {
    openSyncModal();
    document.getElementById("btnSyncApply").disabled = true;
    syncBody(problem === "not-set"
      ? `<p><b>Sheets sync isn't set up yet.</b></p>
         <p>The <code>googleClientId</code> field in <code>firebase-config.js</code> is still
         the placeholder, so there's nothing to connect to. README Part 5 walks through
         creating one in the Google Cloud console.</p>
         <p>Everything else in the app works without this.</p>`
      : `<p><b>That doesn't look like a Google client ID.</b></p>
         <p>It should end in <code>.apps.googleusercontent.com</code> and look like:<br>
         <code>1234567890-abc123def456.apps.googleusercontent.com</code></p>
         <p>Currently set to:<br><code>${esc(GOOGLE_CLIENT_ID.slice(0, 60))}${GOOGLE_CLIENT_ID.length > 60 ? "…" : ""}</code></p>
         <p>Check you copied the <b>Client ID</b>, not the client secret or the project number.</p>`);
    return;
  }
  ...
  try {
    ...
    const friendly = /invalid_client|not found/i.test(raw)
      ? "Google didn't recognise the client ID. Check that it's copied in full, that it's the " +
        "Web application OAuth client, and that it belongs to the same Google Cloud project as this Firebase app."
      : /origin|redirect/i.test(raw)
        ? "Google rejected this site's address. Add it under Authorised JavaScript origins — origin only, " +
          "no repo path and no trailing slash."
        : raw;
    syncBody(`<p>Couldn't sync: ${esc(friendly)}</p><p>Nothing was changed.</p>`);
  }
}
...
// ============================================================================
// Item Tags manager — the only place a tag can be created or destroyed.
// ============================================================================
async function refreshTagRegistry() {
  try {
    TAG_REGISTRY = await loadTagRegistry(db);
  } catch (e) {
    console.error("tag registry load failed", e);
    TAG_REGISTRY = [];
  }
  renderCategoryChips();
  if (!document.getElementById("tagModal").classList.contains("hidden")) renderTagManager();
}

function renderTagManager() {
  const counts = usageCounts(ITEMS, TAG_REGISTRY);
  const list = document.getElementById("tagList");
  if (!TAG_REGISTRY.length) {
    list.innerHTML = `<div class="tag-empty">No tags yet. Add your first one above.</div>`;
    return;
  }
  list.innerHTML = TAG_REGISTRY.map((t) => {
    const n = counts.get(t.name) || 0;
    return `<div class="tag-row">
      <span class="tname">${esc(t.name)}</span>
      <span class="tcount">${n} item${n === 1 ? "" : "s"}</span>
      <button class="icon-btn" data-del-tag="${esc(t.name)}" aria-label="Delete ${esc(t.name)}">🗑</button>
    </div>`;
  }).join("");
}

function openTagModal() {
  if (!isManager) { openLoginModal("Sign in to manage tags."); return; }
  // A missing element here used to mean the button simply did nothing, which is
  // indistinguishable from a dead button. Say what's actually wrong instead.
  const modal = document.getElementById("tagModal");
  if (!modal) {
    toast("Tag editor markup missing — re-upload index.html");
    console.error("#tagModal not found. index.html is out of date relative to app.js.");
    return;
  }
  document.getElementById("tagAddError").classList.add("hidden");
...
function bind(id, event, handler) {
  ...
  if (!el) { console.warn(`Missing element #${id} — index.html may be out of date.`); return; }
  el[event] = handler;
}

bind("btnItemTags", "onclick", openTagModal);
bind("btnCloseTagModal", "onclick", () => {
  const m = document.getElementById("tagModal");
  if (m) m.classList.add("hidden");
});

bind("btnAddTag", "onclick", async () => {
  const input = document.getElementById("newTagName");
  const err = document.getElementById("tagAddError");
  const raw = input.value;
  const problem = validateTagName(raw);
  if (problem) { err.textContent = problem; err.classList.remove("hidden"); return; }
  const name = normalizeTagName(raw);
  if (TAG_REGISTRY.some((t) => t.name === name)) {
    err.textContent = `"${name}" already exists.`; err.classList.remove("hidden"); return;
  }
  err.classList.add("hidden");
  try {
    await createTag(db, name);
    input.value = "";
    await refreshTagRegistry();
    renderTagSuggestions();
  } catch (e) {
    console.error(e);
    err.textContent = e.code === "permission-denied"
      ? "Rejected by the database — publish the updated rules with the tags block."
      : e.message || "Couldn't add that tag.";
    err.classList.remove("hidden");
  }
});

bind("newTagName", "onkeydown", (e) => {
  if (e.key === "Enter") { e.preventDefault(); document.getElementById("btnAddTag").click(); }
});

bind("tagList", "onclick", async (e) => {
  const btn = e.target.closest("[data-del-tag]");
  if (!btn) return;
  const name = btn.dataset.delTag;
  const counts = usageCounts(ITEMS, TAG_REGISTRY);
  const n = counts.get(name) || 0;
  // Deleting a tag strips it from every item, so the count is stated plainly
  // rather than buried — this is not an undoable action.
  const msg = n === 0
    ? `Delete the tag "${name}"?`
    : `"${name}" is used by ${n} item${n === 1 ? "" : "s"}. Deleting it removes the tag from ${n === 1 ? "that item" : "those items"}. Continue?`;
  if (!confirm(msg)) return;
  try {
    await deleteTag(db, name, ITEMS);
    formTags = formTags.filter((t) => t !== name);
    selectedFilterTags = selectedFilterTags.filter((t) => t !== name);
    await refreshTagRegistry();
    renderChosenTags();
    renderGrid(true);
    toast(`Deleted "${name}"`);
...
// ---- one-time migration off the old category field ----
function refreshMigrateTagsButton() {
  const btn = document.getElementById("btnMigrateTags");
  if (!btn) return;
  const n = isManager ? ITEMS.filter((it) => it.category && !(it.tags || []).length).length : 0;
  btn.classList.toggle("hidden", n === 0);
  btn.textContent = `🔀 Convert ${n} categor${n === 1 ? "y" : "ies"} to tags`;
}

bind("btnMigrateTags", "onclick", async () => {
  const btn = document.getElementById("btnMigrateTags");
  const preview = categoriesToTags(ITEMS);
  if (!confirm(`This creates ${preview.tags.length} tag(s) from your existing categories and applies them to ${preview.perItem.size} item(s). Multi-word categories split into separate tags. Continue?`)) return;
  btn.disabled = true;
  try {
    const res = await migrateCategories(db, ITEMS, (d, t) => { btn.textContent = `Converting ${d}/${t}…`; });
    await refreshTagRegistry();
    toast(`Created ${res.tagsCreated} tag(s) across ${res.itemsUpdated} item(s)`);
  } catch (e) {
    console.error(e);
    toast("Conversion failed — nothing was lost, try again");
  } finally {
    btn.disabled = false;
    refreshMigrateTagsButton();
  }
});
