...
import {
  ...
  collection, doc, onSnapshot, setDoc, updateDoc, deleteDoc, increment, writeBatch,
...
function flushStockWrites() {
  ...
  work.forEach(([id, delta]) => {
    ...
    updateDoc(itemDocRef(id), { stock: increment(delta) }).catch((e) => {
      console.error(e);
      toast(e.code === "permission-denied" ? "Sign in required to change stock" : "Couldn't save stock change");
    });
  });
}
...
function openEditModal(it) {
  ...
  formPhoto = existing ? { ...existing, unchanged: true }
    : it.image ? { thumb: it.image, full: it.image, avgRgb: it.avgRgb, unchanged: true }
    : null;
  if (!formPhoto && it.hasPhoto) {
    // Not in cache yet — fetch it so an edit doesn't silently drop the photo.
    loadPhoto(db, it.id).then((ph) => {
      if (!ph || editingId !== it.id) return;
      formPhoto = { ...ph, unchanged: true };
...
async function submitForm() {
  ...
  try {
    ...
    if (formPhoto && !formPhoto.unchanged) {
      await savePhoto(db, item.id, { thumb: formPhoto.thumb, full: formPhoto.full, avgRgb: formPhoto.avgRgb });
    ...
    if (renamedFrom) {
      ...
      if (!ok) return;
      if (!formPhoto) await deletePhoto(db, item.id); // photo removed from the item
    }
